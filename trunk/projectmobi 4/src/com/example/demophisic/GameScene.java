					...
					public IEntity onLoadEntity(
						...
						try {
							MWord = MusicFactory.createMusicFromAsset(
									engine.getMusicManager(), activity, "mfx/ogg/words/"+keyword+".ogg");
						} catch (IOException e) {
							e.printStackTrace();
						}
						MWord.play();
...
private Sprite clock;
	...
	private  static int timeRemaining = 40;
	//count time
				...
				if (isIstouch() == false) {
					...
					try {
						MusicChar = MusicFactory.createMusicFromAsset(
								engine.getMusicManager(), activity, "mfx/ogg/alphabet/"+this.getmText()+".ogg");
					} catch (IOException e) {
						e.printStackTrace();
					}
					MusicChar.play();
